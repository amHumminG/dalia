...
_The central interface and lifetime manager for the DALIA audio engine._ 
...
|  [**Result**](namespacedalia.md#enum-result) | [**GetActiveOutputDeviceInfo**](#function-getactiveoutputdeviceinfo) ([**OutputDeviceInfo**](structdalia_1_1OutputDeviceInfo.md) & info) const<br>_Requests the metadata for the active output device._  |
|  [**Result**](namespacedalia.md#enum-result) | [**GetOutputDeviceCount**](#function-getoutputdevicecount) (uint32\_t & count) const<br>_Queries the OS for available output devices and retrieves the total count._  |
|  [**Result**](namespacedalia.md#enum-result) | [**GetOutputDeviceInfo**](#function-getoutputdeviceinfo) (uint32\_t index, [**OutputDeviceInfo**](structdalia_1_1OutputDeviceInfo.md) & info) const<br>_Queries the metadata for a specific output device from the internal cache._  |
...
|  [**Result**](namespacedalia.md#enum-result) | [**GetTargetOutputDeviceId**](#function-gettargetoutputdeviceid) (char \* identifier, size\_t maxLength) const<br>_Queries the device identifier of the currently targeted output device._  |
|  [**Result**](namespacedalia.md#enum-result) | [**Initialize**](#function-initialize) (const [**EngineConfig**](structdalia_1_1EngineConfig.md) & config=[**EngineConfig**](structdalia_1_1EngineConfig.md){}) <br>_Initializes the engine, hardware device and background threads._  |
...
|  [**Result**](namespacedalia.md#enum-result) | [**SetOutputDevice**](#function-setoutputdevice) (const char \* identifier) <br>_Requests a manual change of output device._  |
...
### function GetActiveOutputDeviceInfo 

_Requests the metadata for the active output device._ 
```C++
Result dalia::Engine::GetActiveOutputDeviceInfo (
...
* `info` The struct to be populated with the output device metadata.
...
* Result::Ok The device metadata was successfully retrieved. 
* Result::NotInitialized The engine is not initialized. 
* Result::StateCorrupted Internal engine state is corrupted. 
...
### function GetOutputDeviceCount 

_Queries the OS for available output devices and retrieves the total count._ 
```C++
Result dalia::Engine::GetOutputDeviceCount (
    uint32_t & count
) const
```
...
This function forces the engine to scan for all available audio devices and refresh its internal cache. It is recommended to call this function once before looping through devices with [**GetOutputDeviceInfo()**](classdalia_1_1Engine.md#function-getoutputdeviceinfo) to ensure that the device list is up to date.
...
* `count` Variable to be populated with the total number of available output devices.
...
* Result::Ok The device count was successfully retrieved and the internal cache was updated. 
* Result::NotInitialized The engine is not initialized. 
...
_Queries the metadata for a specific output device from the internal cache._ 
```C++
Result dalia::Engine::GetOutputDeviceInfo (
    uint32_t index,
    OutputDeviceInfo & info
) const
```
...
* `index` The zero-based index of the device to query. 
* `info` The struct to be populated with the output device metadata.
...
* Result::Ok The device metadata was successfully retrieved. 
* Result::NotInitialized The engine is not initialized. 
* Result::InvalidArgs The provided index is out of bounds. 
...
### function GetTargetOutputDeviceId 

_Queries the device identifier of the currently targeted output device._ 
```C++
Result dalia::Engine::GetTargetOutputDeviceId (
    char * identifier,
    size_t maxLength
) const
```
...
If a device swap has been requested, this represents the requested output device, which may differ from the current output device.
...
* `identifier` A pointer to the buffer where the null-terminated target device identifier will be written. If the engine is targeting the default OS output device. "default" will be written to the buffer. 
* `maxLength` The total capacity of the provided buffer in bytes (including space for the null-terminator). It is recommended to use MAX\_DEVICE\_STR\_LEN as buffer size.
...
* Result::Ok The device metadata was successfully retrieved. 
* Result::NotInitialized The engine is not initialized. 
* Result::InvalidArgs The provided buffer pointer is nullptr or maxLength is 0. 
...
```C++
Result dalia::Engine::Initialize (
...
This function allocates all internal memory pools and starts the background audio thread, async loading thread, async streaming thread, and async control thread. It must be called once before any other engine methods are used.
...
_Requests a manual change of output device._ 
```C++
Result dalia::Engine::SetOutputDevice (
    const char * identifier
) 
```
...
* `identifier` A null-terminated string containing the unique hardware identifier of the target device. Passing a nullptr or an empty string will automatically revert the routing to the OS default device.
...
* Result::Ok The request swap was successfully queued or is already the active output device. 
* Result::NotInitialized The engine is not initialized. 
...
------------------------------
The documentation for this class was generated from the following file `dalia/Engine.h`
