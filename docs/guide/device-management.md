...
!!! info "Platform Support"
    Device management is currently only implemented for the Windows WASAPI driver.
...
### Requesting a Swap
Once you have the identifier of the desired output device, you can request a swap. *Note: Audio device swapping is an async
operation and will never block the calling thread.*
```c++
...
output device is available, the engine still renders audio to the "void" as if it was playing to an actual device. This
